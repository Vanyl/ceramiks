...
const Profile = () => {
    ...
    const DeleteAccountModal = ({ isOpen, onRequestClose, onConfirm }) => {
        return (
            <Modal
                isOpen={isOpen}
    ...
    useEffect(() => {
        const hasInProcessOrders = orders.some(order => order.order_status.trim() === 'En Cours');
        if (hasInProcessOrders) {
            setIsDisabled(true);
            setTooltipMessage("Can't delete account if there's order in process. Please cancel them.");
        } else {
            setIsDisabled(false);
            setTooltipMessage('');
        }
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            const response = await fetch(`https://ecommerce-website3333-593ff35538d5.herokuapp.com/user/delete/${authState.id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${authState.token}`
